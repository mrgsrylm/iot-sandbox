...
public class ChallengeAttemptControllerTests {
    ...
    void postValidResult() throws Exception {
        // given
        User user = new User(1L, "budi");
        long attemptId = 5L;
        ChallengeAttemptDTO attemptDTO = new ChallengeAttemptDTO(50, 70, "budi", 3500);
    ...
    void postInvalidResult() throws Exception {
        ...
        ChallengeAttemptDTO attemptDTO = new ChallengeAttemptDTO(2000, -70, "budi", 1);
        // when
        MockHttpServletResponse response = mvc
                .perform(post("/attempts").contentType(MediaType.APPLICATION_JSON)
                                .content(jsonRequestAttempt.write(attemptDTO).getJson()))
    ...
    public void getUserStats() throws Exception {
        ...
        given(challengeService.getStatsForUser("budi_do")).willReturn(recentAttempts);
        // when
        MockHttpServletResponse response = mvc
                .perform(get("/attempts").param("alias", "budi_do"))
                .andReturn().getResponse();
        // then
        then(response.getStatus()).isEqualTo(HttpStatus.OK.value());
        then(response.getContentAsString()).isEqualTo(
